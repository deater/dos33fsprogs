...
zx02_full_decomp:
	...
	sty	offset		; clear out offset
	sty	offset+1

	sty	ZX0_dst		; low byte destination (always on page boundary)
...
cop1:
	...
	ldy	pntr		; if >192 clamp to 191			; 4
	cpy	#192		; assume output mirrors that in gap	; 2
...
pntr_good:
					; Ypos value is in Y
	lda	hposn_low,Y		; lookup location of row X	; 4+
	sta	FAKEL							; 3
	lda	hposn_high,Y						; 4+
	sta	FAKEH							; 3

	ldy	pntr+1			; get Xpos value		; 4
...
pntr_noflo:
	...
	jsr	store_and_inc		; also restores Y=0		; 6+??
