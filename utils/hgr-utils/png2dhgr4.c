/* Converts 140x192 8-bit PNG file with correct palette to */
/* compact 4-color Apple II DHGR */

/* colors should be 0 / 5 / 7 / 15 */
/* black, grey, light blue, white */
...
//
//  4 bytes = 7 pixels.  20 bytes across display
//  2 bits per color, so 140/4 = 35 (35 bytes per row)


	sprintf(outfile,"%s.packed",base);
	aux=fopen(outfile,"w");
	if (aux==NULL) {
